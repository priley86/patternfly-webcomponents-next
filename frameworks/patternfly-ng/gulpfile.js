...
function copyToBuild(srcArr) {
  return gulp.src(srcArr.concat(globalExcludes)).pipe(
...
gulp.task('build', ['transpile', 'copy-css', 'copy-html', 'copy-static-assets'])
...
gulp.task('inline-template', ['transpile-less'], function() {
  return gulp
    .src(['./src/app/**/*.ts', './src/app/**/*.js'].concat(globalExcludes), {
      base: './'
    })
    .pipe(
      replace(/templateUrl.*\'/g, function(matched) {
        var fileName = matched.match(/\/.*html/g).toString()
        var dirName = this.file.relative.substring(
          0,
          this.file.relative.lastIndexOf('/')
        )
    ...
    .pipe(
      replace(/styleUrls.*\'/g, function(matched) {
...
gulp.task('copy-root', function() {
  return gulp
    .src(['index.ts', 'patternfly-ng.module.ts'])
    .pipe(gulp.dest(libraryBuild))
})

gulp.task('copy-wcs', function() {
  return gulp
    .src(['src/app/web-components'])
    .pipe(gulp.dest(libraryBuild + '/src/app'))
})

gulp.task('copy-html', function() {
  return copyToDist(['src/**/*.html'])
})

gulp.task('copy-css', function() {
  return copyToDist(['src/**/*.css'])
})

gulp.task('copy-examples', function() {
  return copyToDemo(['src/**/examples/*.*'])
})

gulp.task('copy-static-assets', function() {
  return gulp.src(['LICENSE', 'README.md']).pipe(gulp.dest(libraryDist))
})
...
gulp.task('watch', ['build', 'copy-watch-all'], function() {
  gulp
    .watch(
      ...
      ['transpile', 'post-transpile', 'copy-watch']
    )
